...
async def async_setup_entry(
    ...
    # Cleanup old carbon footprint coordinator if exists
    carbon_key = f"{entry.entry_id}_carbon_footprint"
    carbon_coordinator = hass.data[DOMAIN].get(carbon_key)
    if carbon_coordinator:
        _LOGGER.debug("Cleaning up existing carbon footprint coordinator")
        if hasattr(carbon_coordinator, '_unsub_hourly') and carbon_coordinator._unsub_hourly:
            carbon_coordinator._unsub_hourly()
        if hasattr(carbon_coordinator, '_unsub_midnight') and carbon_coordinator._unsub_midnight:
            carbon_coordinator._unsub_midnight()
        hass.data[DOMAIN].pop(carbon_key, None)
